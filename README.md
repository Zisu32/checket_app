...
* **Installations-Hinweis (iOS/Android)**: Dismissbarer Banner im Kunden-Ticket, solange die Seite nicht bereits als installierte PWA läuft. Bleibt nach dem Wegwischen dauerhaft verborgen.
## Architektur & Konzept
...
* **`shared/services/platform_hints.dart`**: Erkennt, ob das Gerät iOS oder Android ist und ob die Seite bereits im Standalone-Modus (als installierte PWA) läuft. Steuert darüber den dismissbaren Installations-Hinweis-Banner im Kunden-Ticket. Hintergrund: Web-Push-Notifications sind auf iOS – in jedem Browser, nicht nur Safari – ausschließlich innerhalb einer zum Home-Bildschirm hinzugefügten Web-App verfügbar; ein normaler Browser-Tab hat dort keinen Zugriff auf die Notification-API, weshalb der Banner gezielt zur Installation hinführt.
* **`shared/theme/app_theme.dart`**: Zentrale FontSize und Farbdefinitionen für alle Status (aktiv, unbezahlt, temporär, vergessen, frei) sowie Hintergrund-/Oberflächenfarben, damit Dashboard, QR-Monitor und Kunden-Ticket konsistent aussehen.
...
* **`staff_app/views/qr_display_view.dart`**: Die zweite Ansicht der Staff-App, läuft im separaten "Checket Monitor"-Tab. Zeigt Bügelnummer + QR-Code für den aktuell aktiven Vorgang oder einen generischen "Ticket wiederherstellen"-Code, und aktualisiert sich per `MonitorService.onUpdate` sowie beim Start über `readLastKnown()`.
* **`customer_app/views/webticket_view.dart`**: Die Ticket-Ansicht für den Kunden. Abonniert `SyncService.watchTicket(id, secret)`, zeigt animierten Status (aktiv, unbezahlt, temporär draußen, im Fundbüro, abgeholt, Bügel frei), fragt optional Push-Berechtigung an, zeigt bei Bedarf den Installations-Hinweis-Banner (`platform_hints.dart`) und persistiert `id/secret` in `localStorage`, damit der Kunde die Seite ohne Query-Parameter erneut öffnen kann.
