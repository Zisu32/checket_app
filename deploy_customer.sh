...
BUCKET_NAME="checket-customer"
FLUTTER_BIN="/home/zisu32/Android/flutter/bin/flutter"

echo "Starte Build für Checket (Customer)..."
$FLUTTER_BIN build web -t lib/main_customer.dart --release --base-href "/storage/v1/object/public/$BUCKET_NAME/"

if [ $? -eq 0 ]; then
    echo "Upload zu Supabase läuft (mit expliziten MIME-Types)..."

    # Wir löschen NICHT mehr den ganzen Bucket, um die "Public"-Einstellung zu erhalten.
    # Stattdessen überschreiben wir Dateien oder löschen sie einzeln bei Konflikten.
